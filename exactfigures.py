...
N = 1001
xx = linspace(0.0,exactN.xg,N)

HH, hx, uu, MM = exactN.exactN(xx)
_, BB = exactN.exactNbueler(xx)

# surface elevation is so that ice reaches flotation exactly at xg
hh = HH - (exactN.rho/exactN.rhow) * exactN.Hg
...
xl = linspace(exactN.xg,exactN.L0,N)
ax1fig1.plot(xl/1000.0, (exactN.rho/exactN.rhow) * exactN.Hg - 30.0 * abs(sin(xl / 4.0e3)),'k',linewidth=0.7*lw)
y1, y2 = ax1fig1.get_ylim()
ax1fig1.set_ylim(0.0,y2)
ax1fig1.set_xlim(0.0,exactN.L0/1000.0)
ax1fig1.plot([exactN.xg/1000.0,exactN.xg/1000.0],[0.0,exactN.Hg],'k',linewidth=lw)
ax1fig1.text((exactN.xg-10.0e3)/1000.0,-200.0,r"$x_g$",fontsize=18)
ax1fig1.plot([0.0,8.0],[exactN.bg,exactN.bg],'k',linewidth=0.5*lw)
ax1fig1.text(+12.0,0.8*exactN.bg,r"$b_g$",fontsize=18)
ax1fig1.hold(False)
...
ax1fig2.text((exactN.xg-10.0e3)/1000.0,-0.25,r"$x_g$",fontsize=18)
ax1fig2.hold(False)
