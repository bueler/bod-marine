...
\subsection*{Verification of a grid-free ``shooting'' numerical method}  In our steady flowline case the model equations have the form of a two-point boundary value problem for ordinary differential equations (ODEs).  Specifically, the system of three first-order ODEs \eqref{eq:steadymass}--\eqref{eq:Tstress} has two boundary conditions at $x=0$, namely equations \eqref{eq:leftbc}, and one at $x=x_c$, the calving-front stress condition \eqref{eq:rightbc}.  

Such a problem allows a nonlinear ``shooting'' method \citep[section 17.1]{Pressetal}.  For this technique we use the correct values for $u(0)$ and $H(0)$ from \eqref{eq:leftbc} and guess an additional value $T_0$ for $T(0)$.  Then we apply a standard ODE initial value problem (IVP) solver to compute solutions $(\tilde u(x),\tilde H(x),\tilde T(x))$ from $x=0$ to $x=x_c$.  The failure of these ODE IVP solutions to fit boundary condition \eqref{eq:rightbc} is a measure of the wrongness of $T_0$.  If we define this function
\begin{equation}
F(T_0) = \tilde T(x_c) - \frac{1}{2} \omega \rho g \tilde H(x_c)^2
\end{equation}
we can apply a numerical method to find the root $\hat T_0$ to the problem $F(T_0)=0$.  The solution $T_0$ to the problem $F(T_0)=0$ gives us initial conditions so that the ODE IVP solution $(u,H,T)$ solves the complete two-point boundary value problem \eqref{eq:steadymass}--\eqref{eq:rightbc}.

A robust root-finding method, which avoids computing $F'$, is bisection \citep[section 9.1]{Pressetal}.  Note $F'$ may not exist in this case because of the low regularity of the solution at the interior point $x=x_g$.  Observe that in this case it is easy to get an initial bracket, that is, values of $T_0$ which make $F$ have different signs, for starting the bisection iteration.  We will see, because we have an exact solution, clear evidence that the bisection iteration succeeds despite the low reqularity of the ODE solution.
...
\subsection*{Linearization around the exact solution}
FIXME: why stiff when grounded and non-stiff when floating?  look at linearization
