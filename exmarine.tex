...
FIXME:  When one does nonlinear shooting using the exact correct $T$ value, one gets the best-case adaptive-grid error in Figure \ref{fig:shoot-error}.  Used with high tolerances: rtol = $10^{-12}$ and atol = $10^{-14}$.  The ode solver detects that the grounding line should involve shorter steps, but more importantly the grounded ice requires a stiff method while the floating is nonstiff; see Figure \ref{fig:shoot-dt-adaptive}.
