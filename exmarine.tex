...
The result of the above construction is the following set of formulas for an exact marine ice sheet satisfying our steady model equations \eqref{eq:steadymass}--\eqref{eq:xgregularity}.  The velocity comes from combining the Bodvardsson and van der Veen results,
\begin{equation}
...
where
\begin{equation}
C_s = \left(\frac{\rho g \omega}{4 B(x_g)}\right)^n.  \label{eq:vanderveenconstant}
\end{equation}
and where $u_s(x)$ is defined by
\begin{equation}
u_s(x)^{n+1} = u(x_g)^{n+1} + \frac{C_s}{M(x_g)} \left(\left[Q_g + M(x_g) (x-x_g)\right]^{n+1} - Q_g^{n+1}\right) \label{eq:vanderveenvel}
\end{equation}
where $Q_g = u(x_g) H(x_g)$.  Similarly the thickness comes from combining the results:
\begin{equation}
...
\end{equation}
Formulas \eqref{eq:marinevel} and \eqref{eq:marinethickness} define the continuous functions which are shown in Figure \ref{fig:exactmarine}, using the specific values in Table \ref{tab:exactsolnnotation}.

%\small
\begin{table}
\caption{Additional specific values used in the exact solution shown in Figures \ref{fig:exactmarine}--\eqref{fig:exactmarine-detail}.}\label{tab:exactsolnnotation}

\medskip
\begin{tabular}{llll}
Symbol & Description & Units \\ \hline
FIXME $x_g$ & grounding line & m  \\
FIXME $z_o$ & ocean surface elevation & m \\
\end{tabular}
\end{table}
%\normalsize
...
Note that, because this paper is focussed on ice flow dynamics and grounding lines, we treat $M(x)$ as a predetermined field (i.e.~the one shown in Figure \ref{fig:exactMB}).  This removes the climatically-interesting elevation--accumulation feedback, and the associated instability, which was considered by Bodvardsson, Weertman, and others.  This feedback can be added back by restorig the elevation-dependence of the mass balance, that is, by using equation \eqref{eq:bodmassbalance} to determine $M$ from $H(x)$.
