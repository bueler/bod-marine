...
\usepackage{natbib}  % see igs2eguide.tes for example citation styles
...
\title{On perfect-plasticity, plug flows, and parabolic profiles}
...
\subsection*{Introduction}  At its beginning, theoretical glaciological offered-up two parabolic profiles as the shapes of steady ice sheets lying on flat beds, as in Figure \ref{fig:twoparabolas}.  One was the profile of an ice sheet with perfectly-plasticity \citep{Orowan,Nye52plastic} and the other the profile for a sliding (plug) flow investigated by \cite{Bodvardsson}.  These were among the earliest ``global'' views of the free surface flows in glaciology.  They focussed on different aspects of the global problem, and they came to very different conclusions.  These authors found parabolas which exactly solved their nonlinear flow equations, but their solutions are rarely used for modeling glacier and ice sheet flow in recent decades.\footnote{Draft date \emph{\today}.}  % <-- IS THIS TRUE HISTORICALLY

The profiles, shown in Figure \ref{fig:twoparabolas}, are remarkably different curves, especially in their descriptions of the dome (the highest point of the ice sheet) and margin (where the ice thickness goes to zero) shape.   Up to scaling, one is of the form $x=1-y^2$ (perfect-plasticity) and the other is of the form $y=1-x^2$ (the plug flow).  The perfect-plasticity solution has a central peak at the highest point of the ice sheet, and a margin with unbounded surface gradient.  The plug flow solution has a smooth dome and a finite-slope, wedge-shaped margin.

The purpose of this paper is to explain to the modern glaciologist how these two classic parabolas arise as limits of the physics in the standard SIA and SSA stress balance models, when coupled with the mass continuity equation.  We have identified a new connection between similarity solutions of the SIA and the perfectly-plastic model, and this connection illuminates the time-dependent behavior of nearly-plastic ice.  And it turns out that the Bodvardsson parabola directly relates to an important recent ice sheet modeling problem, the modeling of marine ice sheets in the rapidly-sliding case.

\begin{figure}[ht]
\onecol{twoparabolas}
\caption{The parabolas offered by Orowan and Nye (1949; 1952) (solid) and by \cite{Bodvardsson} (dashed) as the profiles of steady ice sheets on flat beds.  Here each ice sheet is symmetric, so only a flow line from dome to margin is shown.  A dome thickness of $H_0=3000$ m and a total flowline distance of $L_0=500$ km (from dome to margin where $H=0$) are chosen for concreteness in this paper.} \label{fig:twoparabolas}
\end{figure}

\subsection*{Continuum models}

FIXME: STATE MASS CONTINUITY, GLEN LAW, SIA, SSA

\subsection*{Perfect-plasticity and the SIA}

\subsubsection*{A time-dependent approach to perfect plasticity}
The geometry of a ice sheet or glacier formed from hypothetical perfectly-plastic ice has no time-dependence.  Indeed, the geometry of such an ice sheet does not depend on the mass balance either.  

This paper will cite \citep{BLKCB,Halfar81,Halfar83}
...
$A$ & ice softness & $\text{Pa}^{-3}\,\text{s}^{-1}$  \\
...
$H$ & ice thickness and ice surface elevation & m \\
$M$ & ice-equivalent surface mass balance & $\text{m}\,\text{s}^{-1}$ \\
$n$ & Glen exponent in ice flow law & 3 \\
$\nu$ & vertically-averaged (effective) viscosity & Pa s \\
 & \quad  in SSA & \\
...
\newcommand{\dezero}{\dot \eps_0}
The flow law is converted from the classic form $D_{ij} = A |\tau|^{n-1} \tau_{ij}$ to the more effective form (for seeing the plastic limit),
	$$D_{ij} = \dezero \frac{|\tau|^{n-1} \tau_{ij}}{\tau_0^n}$$
by the conversion
	$$\dezero = A \tau_0^n.$$

In order to take the $n\to\infty$ limit, we must have $\tau_0 = \rho g H_0 / (2 R_0)$, exactly as expected for plasticity.  In this plastic limit it follows that
% see equation 1.4.4 in Oer...
	$$h(x)^2 - H_0^2 = \frac{2 \tau_0}{\rho g} (x - x_0).$$
Such an equation appears in \cite{Nye52plastic} and in more recent texts
\cite[equation 1.4.4]{Oerlemans2008minimal}.  FIXME: form with $L$ and $H_0$ needed for reference from Figure \ref{fig:twoparabolas}.

\begin{figure}[ht]
\onecol{figure1}
\caption{The $t_0$ profiles of the Halfar solutions for $n=3,10,50$, and for the plastic limit ($n=\infty$).  The $n=50$ and plastic ($n\to\infty$) profiles are essentially co-incident at display resolution.} \label{fig:figure1}
\end{figure}

\begin{figure}[ht]
\onecol{plhalfar}
\caption{The black profiles are as in Figure \ref{fig:figure1}, but now the past profiles (period from $10^{-3} t_0$ to $t_0$) are shown as green shading and the future profiles ($t_0$ to $10^3 t_0$) as red shading.  The $n=\infty$ result would have no separation of past, present, and future, so it is not shown here.} \label{fig:plhalfar}
\end{figure}

\emph{Summary}: If $n$ is large then the ice sheet spends most of its time with geometry close to the $t_0$ shape.  This is the answer to the not-quite-absurd question, ``what is the time-evolution of the perfectly-plastic ice sheet?''

In fact, in the ``high $n$'' case we can describe the life of the similarity solution as ``at time zero there is a delta-function of ice at $x=0$; an infinitesimal time later the profile is the plastic one shown in Figure 2; an eternity later the ice sheet shrinks to zero thickness.''  Such a description effectively applies to all ice sheets governed by high $n$ values.  As has been exploited a number of times, Nye's (1952b)\nocite{Nye52plastic} strategy allows us to reconstruct such an ice sheet from its margin outline, for example CITE LINGLE RECONSTRUCTION

FOR THE HALFAR SOLUTION, GIVE A THEOREM THAT $H^{(2n+2)/n}$ IS CLOSE TO THE $t_0$ PROFILE FOR MOST OF ITS LIFE, AS $n\to \infty$

THE \cite{BLKCB} SOLUTIONS WITH $\lam\ne 0$ ARE NOT SO SIMPLE BECAUSE DEVIATIONS FROM $t_0$ SHAPE YIELD MAJOR DIFFERENCES IN MASS BALANCE (?); ALSO HEAD FOR THE PERFECTLY-PLASTIC CASE, SHOWING LESS AND LESS DEPENDENCE ON THEIR MASS BALANCE?


It is well-known that a perfectly-plastic ice sheet has no time-dependence.  We see here, by considering the $n\to\infty$ limit both in the flow law and in the corresponding Halfar flow (similarity) solutions, that nearly-plastic ice sheets spend almost all their lives close to the plastic geometry, with only a very brief period of flow to reach that state, and an agonizingly-slow decay from that state when the only mechanism for thinning is flow.
...
\begin{figure}[ht]
\onecol{bodverifthickvel}
\caption{Thickness $H(x)$ and horizontal velocity $u(x)$ in verification case of the Bodvardsson solution (Test N).  The calving-front stress boundary conditions is imposed at $x=x_c$ where the ice exactly reaches flotation.  The ice surface elevation satisfies $h=H-500$, and th elevation $h=0$ of the sea is shown.} \label{fig:bodverifthickvel}
\end{figure}

\begin{figure}[ht]
\onecol{bodverifbetaB}
\caption{Sliding law parameter $\beta(x)$, equation \eqref{betadefine}, and ice hardness $B(x)$, equation \eqref{hardnessdefine}, for the Bodvarssson verification case (Test N).} \label{fig:bodverifbetaB}
\end{figure}

\subsubsection*{An exact solution to the flowline marine ice sheet problem}  YOU CAN GLUE THE \cite{vanderVeen85} (\& SANDERSON?) STEADY ICE SHELF ON THE END AND IT IS THEN AN EXACT MARINE ICE SHEET; SHOW
...
\subsection*{Conclusions} BLAH BLAH.  If one wanted a model that included both shapes, try \citep{BBssasliding}.


\subsection*{Acknowledgements}  Thanks to Heinz Blatter and Helgi Bj\"ornsson for tracking down the original paper by Bodvardsson, in addressing an unrelated matter.

\subsection*{Erratum and historical note}   \cite{BLKCB} identify the constant accumulation SIA solution as ``Bodvarsson (1955)--Vialov (1958).''  This is incorrect.  It is attributable only to Vialov.  Also, the spelling of Bodvardsson's name is ``Bodvarsson'' in many places, but the original paper in 1955 has ``Bodvardsson'' with a ``d''.  The \cite{Weertman61stability} response to Bodvardsson's work, the most significant such response, drops the ``d''.
