...
plt.subplot(2,1,1)
plt.semilogy((x-xa)/1000.0,abs(H-Hexact)/Hexact,'k',lw=2.0)
ax1 = plt.gca()
ax1.set_xlim([0.0,(xoceanend-xa)/1000.0])
plt.grid(True)
plt.ylabel(r"$|H - H_{exact}|/|H_{exact}|$")
plt.subplot(2,1,2)
plt.semilogy((x-xa)/1000.0,abs(u-uexact)/uexact,'k',lw=2.0)
ax2 = plt.gca()
ax2.set_xlim([0.0,(xoceanend-xa)/1000.0])
plt.grid(True)
plt.ylabel(r"$|u - u_{exact}|/|u_{exact}|$")
plt.xlabel("x   (km)")
