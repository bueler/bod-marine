...
/* ------------------------------------------------------------------------
    ...
    This example shows how geometric multigrid can be run transparently with a
    nonlinear solver so long as SNESSetDM() is provided.  Example usage
    ...
    or to run with grid sequencing on the nonlinear problem (note that you do
    not need to provide the number of multigrid levels, it will be determined
    automatically based on the number of refinements done)
...
int main(int argc,char **argv)
{
  ...
  PetscReal      fnorm;                        /* final function norm */
  PetscErrorCode ierr;
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     Solve nonlinear system and quick report
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = SNESSolve(snes,NULL,x);CHKERRQ(ierr);
  ierr = SNESGetFunctionNorm(snes,&fnorm);CHKERRQ(ierr);
  ierr = SNESGetIterationNumber(snes,&its);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD,"function norm %.3e after %d iterations\n",fnorm,its);CHKERRQ(ierr);
...
/*
FIXME: is this good doc?  if so, how to use it?
   FormObjectiveLocal - Evaluates objective function (helps line search) on local process patch
 */
  ...
  for (j=info->ys; j<info->ys+info->ym; j++) {
    for (i=info->xs; i<info->xs+info->xm; i++) {
      if (i == 0 || j == 0 || i == info->mx-1 || j == info->my-1) {
        ...
        /* Obj[u] = \int_{\omega} - (1/2) \nabla^2u * u - lambda e^u dx dy
                  = \int_{\omega} (1/2) |\nabla u|^2 - lambda e^u dx dy */

        uxux = u*(2.*u - ue - uw)*hydhx;  /* = - u u_xx dx dy */
...
PetscErrorCode FormJacobianLocal(DMDALocalInfo *info,PetscScalar **x,Mat jacpre,Mat jac,MatStructure *flg,AppCtx *user)
{
  ...
  /*
     Use MatSetValuesStencil() to inserts block of 5 values (typical row) into
     a matrix using structured grid indexing
  */
  for (j=info->ys; j<info->ys+info->ym; j++) {
    for (i=info->xs; i<info->xs+info->xm; i++) {
      row.j = j; row.i = i;
  ...
  /*
     Assemble Jacobian matrix, using the 2-step process
  */
